		...
		if(connection());
					...
					if ($isEndOfColumn) {
					  ...
					  if ($isStartOfNewColum) {
						...
						$num = MysqlSelectQuery("select event_id from events where state='".$rows['state_id']."' and status = 1");
						...
						echo '<li><a href="'.SITE_URL.'events/state/'.$rows['id_state'].'/'.$final.'">'.$rows['name'].'</a></li>';	
