...
Class Convert {
	...
	*/
	...
	{
		$url    = "http://currency-converter.php5developer.com/api.php?from={$from}&to={$to}";
		$amount = (float) $amount;
		...
		if (in_array('curl', get_loaded_extensions())) {
			...
			curl_setopt($ch, CURLOPT_URL, "http://currency-converter.php5developer.com/api.php?from={$from}&to={$to}");
		...
		$this->newCache($from.$to, $response['rate']);
		$amountArr = array('v' => $response['rate']*$amount);
                $response = array_merge($response, $amountArr);
		return $response;		
