...
?>
			...
			?>
                       <div itemscope itemtype="http://schema.org/EducationEvent" class="eventListing" onClick="javascript:url_location('<?php echo SITE_URL.'events/'.$rows['event_id'].'/'.str_replace($title_link,"-",$rows['event_title']);?>')">
                       		<div class="eventListingInner">
                      			<a href="<?php echo SITE_URL.'events/'.$rows['event_id'].'/'.str_replace($title_link,"-",$rows['event_title']);?>" itemprop="url" style="display:block; padding:3px;" title="<?php echo $rows['event_title'];?>">
                                	<span class="spanTitle" itemprop="name" ><?php echo $rows['event_title'];?></span>
                                 </a> 
                      <div class="innerHeadingPropEvent">
     <p itemprop="doorTime" ><?php echo dateDiff($rows['startDate'], $rows['endDate']);?>, 
                       ...
                       <p style="text-align:center;" itemprop="location">
					   <?php echo GetEventLocation($rows['event_id']);?>
                     </p>

                        <div class="respond">
                       <div class="testImg" style="background-image:url(<?php echo SITE_URL.$logo;?>); background-repeat:no-repeat; background-position:center;"></div>
                </div>
...
<p style="text-align:center; font-size:14px; color: #105773; margin:5px 0 5px 0;" >
	<?php echo $rows['organiser'];?>
                        ...
                        <div style="color:#000; font-size:12px; text-align:left;"  class="description" itemprop="description" ><?php echo substr(stripslashes(strip_tags($rows['description'])),0,150).'...';?> </div>
