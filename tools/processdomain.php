...
if(isset($_POST['domain']) && !empty($vpb_domain_name_to_search)) //Be sure that the domain name is set and field is not empty to proceed
{
	...
	$path_parts = pathinfo($vpb_cleaned_domain_name_to_search);
	
	$vpb_cleaned_domain_name_to_search = $path_parts['filename'];
	...
	if($vpb_cleaned_domain_name_to_search != "")
	{
		foreach($vpb_domain_servers as $vpb_domain_servers_ext => $vpb_domain_whois)
		{
			...
			if($vpb_socket = fsockopen($vpb_domain_whois[0], 43))
			{
				...
				{
					echo '<div id="vasplus_pb"><div class="available"><div style="float:left; width:100%; border:0px solid; padding-top:3px; padding-bottom:2px;"><span>Available</span>' . $vpb_cleaned_domain_name_to_search. '<b>' . $vpb_domain_servers_ext .'</b> is Available</div><div style="float:right;" align="right"><a class="vpb_general_button" style="color:#FFF; padding:3px; font-family:Verdana, Geneva, sans-serif; font-size:11px; cursor:pointer; margin:0px;" href="http://www.hostmonster.com/track/nigsemtra2010/GetWebHosting" target="_blank">Get Domain / Web Hosting</a></div><br clear="all" /></div></div><br />';
				}
				else
				{
