			...
			if($rowsBiz['premium'] > 1 && $rowsBiz['premium'] <= 3){
				...
				if($rows_login['expire_status'] == 1 ){
					...
					$_SESSION['login_business'] = false;
						...
						MysqlQuery("UPDATE user_login SET last_login_date='".time()."', browser_info = '".$visitorInfo."' where email='".$_POST['user_email']."'");
						echo "Premium Login";
					}
...
else if($num_rows_subscribers){	//checking if email exist in subscribers table
	if ($rows_subscribers['confirm'] == 0) {
		echo "Unconfirmed User";
	}else {
      ...
      MysqlQuery("UPDATE subscribers SET last_login ='".time()."', browser_info = '".$visitorInfo."' where subscriber_id ='".$rows_subscribers['subscriber_id']."'");
