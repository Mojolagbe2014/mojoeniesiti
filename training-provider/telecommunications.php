...
$paged = ""; $category_query = ""; $title = ""; $pageInnerTitle = ""; $pastLink =""; $pastEvent =""; $today = date("Y-m-d"); $dtds3=date("F"); $advert = ""; $pg = ""; $recordperpage = 30; $pagenum = 1;
if(isset($_GET['page'])){ $pg = " - Pg-".$_GET['page']; $pagenum = $_GET['page']; }
$url = $_SERVER['REQUEST_URI'];$CategoryVal = explode('/',$url);
if(isset($_GET['page'])){$urlVal = explode('?',end($CategoryVal));$url_category = $urlVal[0];} else{ $url_category = end($CategoryVal); }
if(!strpos($url_category,'.php')) {$searchVar = $url_category.".php";} else {$searchVar = $url_category;}
$newFile = 'urlConfigCategory.txt'; $metaFile = '../nstlogin/scripts/category_meta_title_description_training.txt'; $Content = file($newFile);
...
$rowsCT = SqlArrays($resultCT); $query = "and specialization=".$val; $category = $rowsCT['category_name'];
$pageInnerTitle = "Training providers | training institutions |training firms / consultants | professional associations / bodies in ".$category. " category";
...
$title = trimStringToFullWord(60, stripslashes(strip_tags("Training Providers in " .$category ." - Nigerian Seminars and Trainings")));
}
...
<tr><td style="font-size:11px;"><div style="margin-top:2%;text-align:center;color:#fff"><h3 style="font-weight:normal;font-size:11px;"><i>(Search training providers alphabetically)</i></h3></div></td></tr>
</table>
</form>
</div>
<div id="subpage">
