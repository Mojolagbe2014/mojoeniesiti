...
if(isset($_GET['page'])){$urlVal = explode('?',end($CountryVal));$url_country = $urlVal[0];}else{$url_country = end($CountryVal);}
if(!strpos($url_country,'.php')){ $searchVar = $url_country.".php";} else { $searchVar = $url_country; }
$newFile = 'urlConfigCountries.txt'; $metaFile = '../nstlogin/scripts/countries_meta_title_description_training.txt';
...
$offset = ($pagenum - 1) * $recordperpage;
 ...
 if(isset($val) && isset($url_country)){
    $resultCT = MysqlSelectQuery("SELECT * FROM `countries` WHERE id = '".$val."'"); $rowsCT = SqlArrays($resultCT); $query = "and country=".$val;
    $location = $rowsCT['countries']; $pageInnerTitle = "<img src='../images/flags/big/".$img."' style='vertical-align:middle;' alt='flag' /> List of training providers | training institutions |training firms / consultants | professional associations / bodies in ".$location;
...
$result = MysqlSelectQuery("select * from businessinfo where business_type='Training' and status =1 $query order by rand() limit $offset , $recordperpage");
$num = NUM_ROWS($result);
?>
<!DOCTYPE html>
<html lang="en">
...
<tr><td style="font-size:11px;"><div style="margin-top:2%;text-align:center"><h3 style="font-weight:normal;font-size:11px;"><i>(Search training providers alphabetically)</i></h3></div></td></tr>
</table>
</form>
</div>
<div id="subpage">
                            ...
                            <?php 
                                ...
                                foreach($alphas as $alpha){ 
                                    ...
                                    if($num != 0) {
                                        ...
                                        $alphaStyle = 'font-weight: bolder; color:#000;text-decoration:underline';
