...
$add_event = array('entrant_name'=>'','email'=>'','phone'=>'','title'=>'','description'=>'','venue'=>'','category'=>'','cost'=>'','organizer'=>'','facilitator'=>'','start_date'=>'','end_date'=>'','website'=>'','country'=>"",'state'=>"",'verify'=>"","verifyHidden"=>"","tags"=>"");
$business = array("business_name"=>"","email"=>"","description"=>"","address"=>"","size"=>"","capacity"=>"","contact_person"=>"","telephone"=>"","website"=>"","buz_type"=>"",'verify'=>"","verifyHidden"=>"","price"=>"","category"=>"",'country'=>"","designation"=>"");
...
function Get_News(){
	...
	if(connection()){
		...
		while($rows = SqlArrays($result)){
			...
			echo '<li onclick="javascript:url_location(\''.SITE_URL.'news/'.$rows['News_id'].'/'.str_replace($title_link,"-",substr($rows['newsTitle'],0,150)).'\')"><h4 style="font-size:13px; font-weight:normal;"><a href="'.SITE_URL.'news/'.$rows['News_id'].'/'.str_replace($title_link,"-",substr($rows['newsTitle'],0,150)).'/" title="'.$rows['newsTitle'].'" ><span style="color:#0C8EB9;">'.$rows['newsTitle'].'</span></a><span style="display:block;font-size:12px">'.$description.'</span></h4></li>';
			}
...
function Get_Articles(){
	...
	if(connection()){
		...
		$newFile = strtolower($rows['article_id']."-".$newFile);//.".php"
			...
			echo '<li onclick="javascript:url_location(\''.SITE_URL.'articlespg/'.$newFile.'\')"><h4 style="font-size:13px; font-weight:normal;"><a href="'.SITE_URL.'articlespg/'.$newFile.'" title="'.$rows['article_title'].'" ><span style="color:#0C8EB9;">'.$rows['article_title'].'</span></a><span style="display:block;font-size:12px">'.$description.'</span></h4></li>';
...
function createArticles($howMany, $templateName, $outputLoc){
	...
	//create htaccess file if not in directory
		if(!file_exists($genFilesLoc.".htaccess")){
			$fh = fopen($genFilesLoc.".htaccess",'w');
			//the .htaccess rewrite content
			$content = '# Create rule to rewite non .php extension
						RewriteEngine on
						RewriteCond %{REQUEST_FILENAME} !-d 
						RewriteCond %{REQUEST_FILENAME}\.php -f 
						RewriteRule ^([^/]+)/?$ $1.php [NC,L]';
			//write the .htaccess content
			fwrite($fh,$content);
			//close file write
			fclose($fh);
		}
	...
	while($rows = SqlArrays($result)){
		...
		if (copy($sampleFile, $genFilesLoc.$newFile)) {
			...
			$newdata = str_replace("'meta-description'", substr(strip_tags($rows['article_description']),0,130)."-".$rows['article_id'], $newdata); 
